// pages/Tecnico/componente/ModalCalculadorPrecioReparacion.jsx
...
export function ModalCalculadorPrecioReparacion({ show, onHide, onAceptar }) {
  ...
  const cargarPiezasCompatibles = async (idModelo, idCategoria) => {
    ...
    } catch { setError('Error al cargar piezas compatibles'); setPiezas([]); }
  ...
  // ── Devuelve objeto completo con todos los datos necesarios para ReparacionMultiple ──
  const handleAceptar = () => {
    if (!calculo || !piezaSeleccionada) return;
    onAceptar({
      piezaId:     piezaSeleccionada.id_pieza,
      nombrePieza: piezaSeleccionada.nombre_pieza,
      precioPieza: calculo.precioPieza,
      manoObra:    calculo.manoObra,
      costoMano:   calculo.costoMano,
      total:       calculo.total,
    });
    onHide();
  };
    ...
    <Modal show={show} onHide={onHide} size="lg" centered>
      <Modal.Header closeButton className="border-0 pb-0">
        <Modal.Title className="d-flex align-items-center gap-2">
          ...
          Calculador de precio — Pieza
        </Modal.Title>
      </Modal.Header>

      <Modal.Body className="pt-2">
        {error && (
          <Alert variant="danger" dismissible onClose={() => setError(null)} className="py-2">{error}</Alert>
        )}
      ...
      <Modal.Footer className="border-0 pt-0">
        <Button variant="outline-secondary" size="sm" onClick={resetear}
          className="d-flex align-items-center gap-1 me-auto">
          <RefreshCw size={14} /> Limpiar
        </Button>
        <Button variant="secondary" size="sm" onClick={onHide}>Cancelar</Button>
        <Button variant="success" size="sm" onClick={handleAceptar} disabled={!calculo}
